...
struct MeterApator192 : public virtual MeterCommonImplementation {
    MeterApator192(MeterInfo &mi);
...
shared_ptr<Meter> createApator192(MeterInfo &mi)
{
    return shared_ptr<Meter>(new MeterApator192(mi));
}

MeterApator192::MeterApator192(MeterInfo &mi) :
    MeterCommonImplementation(mi, "apator192")
{
...
void MeterApator192::processExtras(string miExtras)
{
    map<string,string> extras;
    bool ok = parseExtras(miExtras, &extras);
    if (!ok)
    {
        error("(apator192) invalid extra parameters (%s)\n", miExtras.c_str());
    }
}

double MeterApator192::totalWaterConsumption(Unit u)
{
    assertQuantity(u, Quantity::Volume);
    return convert(total_water_consumption_m3_*100.0, Unit::M3, u);
}

bool MeterApator192::hasTotalWaterConsumption()
{
    return true;
}

void MeterApator192::processContent(Telegram *t)
{
    ...
    while (i < content.size())
    {
        ...
        if (size == -1 || i+size >= content.size())
        {
            ...
            {
                warning("(apator192) telegram contains a register (%02x) with unknown size.\n"
...
int MeterApator192::registerSize(int c)
{
    switch (c)
    {
